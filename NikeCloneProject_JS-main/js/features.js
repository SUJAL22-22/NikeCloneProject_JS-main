...
function addProductCardEventListeners() {
    ...
    document.querySelectorAll('.add-to-cart').forEach(button => {
        button.addEventListener('click', function() {
            const productId = this.getAttribute('data-product-id');
            handleProductAddToCart(productId, this);
        });
    });
}

function handleProductAddToCart(productId, button) {
    ...
    if (typeof window.addToCart !== 'function') {
        showNotification('Cart functionality is not available right now.');
        return;
    }
    ...
    window.addToCart(product, selectedColor, selectedSize);
